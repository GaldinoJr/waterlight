...
public class GastoHoraDAO  extends AbstractServerDAO {
            ...
            for(int j = 0; j<qtdResidencias; j++) {
                try 
                {
                    ...
                    if(cdResidenciaPesquisa != 0)
                    {
                        ...
                        query = "SELECT gh.cd_gasto_hora, gh.nr_watts, gh.nr_metro_cubico_agua, gh.vlr_gasto_agua, gh.vlr_gasto_luz, gh.cd_residencia, gh.dt_inclusao";
                        ...
                        else
                        {
                            ...
                            {
                                query += " AND DATE_FORMAT(dt_inclusao ,'%d/%m/%Y') = '" + gastoHora.getsDtInclusao() + "'";
                                query += " AND concat(DATE_FORMAT(dt_inclusao ,'%H'), ':', DATE_FORMAT(dt_inclusao,  '%i')) BETWEEN '" + hora + ":00' AND '" + hora + ":59'";
                            }
                        ...
                        for (int i = 0; i < jsonArray.length(); i++) {
                            ...
                            g.setCdResidencia(jsonObject.getInt(g.DF_cdResidencia));
                            if(j == 0) {
                                dataParaComparacao = jsonObject.getString(g.DF_dt_inclusao);
                                hora = extrairHoraDaData(dataParaComparacao);
                            }
                            listGastoHora.add(g);
                        }
                    }
    ...
    private String extrairHoraDaData(String sData)
    {
        int nHora;
        Date data = fomatarData(sData);
        GregorianCalendar calendar = new GregorianCalendar();
        calendar.setTime(data);

        nHora = calendar.get(GregorianCalendar.HOUR);
